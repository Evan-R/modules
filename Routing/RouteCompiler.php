...
class RouteCompiler
{
    ...
    const VAR_REGEXP =  '(?P<%s>%s)';

    const OPT_REGEXP = '(?:%s%s)?';
    ...
    public static function compile(Route $route)
    {
        ...
        if ($route->hasHost()) {
            $results = static::compilePattern($route, $route->getHost(), true);
        ...
        $compiled['tokens']      = $results['tokens'];
        $compiled['static_path'] = $results['staticPath'];
    ...
    {
        if ($req = $route->getHostConstraint($param, null, false)) {
            return $req;
        }

        $route->setHostConstraint($param, static::REQUIREMENTS);
        return false;
    }
    ...
    private static function getVariableTokenRegexp(array $token, array &$tokens)
    {
        ...
        return sprintf(
            ...
            sprintf(static::VAR_REGEXP, $token[3], $token[2])
        );
    }
    ...
    private static function getOptionalTokenRegexp($token, array &$tokens)
    {
        ...
        while (!empty($tokens)) {
            ...
            $option = sprintf(
                static::OPT_REGEXP,
                preg_quote($token[1]),
                sprintf(static::VAR_REGEXP, $token[3], $token[2]).$option
            );
        }
